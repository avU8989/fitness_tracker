...
export async function loadUserPlan(userId: string, planId: string): Promise<PlanDoc | null> {
...
export function findWorkoutDay(plan: any, opts: { dayId?: string, weekId?: string, weekNumber?: number }): {
    day: any | null;
    ...
    if (plan.type.toLowerCase() === "powerlifting") {
        if (opts.weekId) {
            ...
            const wIdx = plan.weeks?.findIndex((w: any) => String(w._id) === String(opts.weekId));
        ...
        if (opts.weekNumber != null) {
            const wIdx = plan.weeks?.findIndex((w: any) => w.weekNumber == opts.weekNumber);
            if (wIdx != null && wIdx > 0) {
                const day = plan.weeks[wIdx].days.id(opts.dayId);
                return { day, container: "weeks.days", weekIdx: wIdx }
